      ...
      <div className="mt-8 grid gap-4">
        {POSTS.map((p) => (
          <Link
            key={p.slug}
            href={`/blog/${p.slug}`}
            className="rounded-2xl bg-white shadow-sm p-5 transition hover:border-lime-400 hover:shadow-sm"
          >
            <h2 className="font-heading text-lg font-semibold text-zinc-900">{p.title}</h2>
